...
const generateMockRecords = (columns, generatedData) => {
    ...
    for (let i = 0; i < MAX_ROWS; i++) {
        ...
        columns.forEach((column) => {
            mockRecord[column.column_name] = generateMockValueForColumn(column, i, generatedData);
        });
...
const generateForeignKeyValue = (foreign_key, generatedData) => {
    ...
    if (validRecords.length !== foreignRecords.length) {
        throw new Error(`Some records in table ${foreign_table_name} are not valid objects.`);
    }
    /* Extract and return a random foreign key value */
    const foreignKeyValues = validRecords.map((record) => record[foreign_column_name]);
