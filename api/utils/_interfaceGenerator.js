...
export function summarizeArrayOfObjectValueTypes(data) {
    ...
    data.forEach((item) => {
        Object.keys(item).forEach((key) => {
            ...
            const valueType = value === null ? 'null' : identifyTSPrimitiveType(value);
...
export function haveSimilarObjects(arr) {
    ...
    const compareStructure = (base, target) => {
        ...
        if (baseType === 'object' && isObject(base) && isObject(target)) {
            ...
            return baseKeys.every((key) => key in target && compareStructure(base[key], target[key]));
        }
 ...
 */
export function generateInterface({ data, interfaceName, isChildObject = false, indentLevel = 1, isDateStringFormat = false, }) {
    const isArrayOfObjectsSimilarType = Array.isArray(data) && haveSimilarObjects(data);
    ...
    const generateInterfaceContent = (obj, optionalKeys) => {
        const getArrayElementType = (arr) => {
            ...
            const allPrimitives = arr.every((item) => item === null ||
                typeof item === 'string' ||
                typeof item === 'number' ||
                typeof item === 'boolean' ||
