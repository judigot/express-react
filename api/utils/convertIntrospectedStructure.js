...
export const getTypeScriptType = (dataType) => {
    ...
    for (const mapping of Object.values(typeMappings)) {
        ...
        if ((Array.isArray(postgresqlIntrospected) &&
...
export const getRequiredColumns = (columns) => columns
    .filter((column) => column.is_nullable === 'NO')
    .map((column) => column.column_name);
export const getForeignTables = (columns) => Array.from(new Set(columns
    .filter((column) => column.foreign_key !== undefined)
    .map((column) => column.foreign_key?.foreign_table_name ?? ''))).filter((tableName) => tableName !== '');
export const getForeignKeys = (columns) => {
    return Array.from(new Set(columns
        .filter((col) => Boolean(col.foreign_key))
        .map((col) => col.column_name)));
};
export const convertColumn = ({ column_name, data_type, is_nullable, column_default, primary_key, unique, foreign_key, }) => {
...
const convertIntrospectedStructure = (tables) => {
    const tableMap = new Map(tables.map((table) => {
        if (isITable(table)) {
