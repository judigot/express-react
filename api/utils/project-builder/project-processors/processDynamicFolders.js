...
export const processDynamicFolders = ({ folderName, children, schemaInfo, schemaInfoParsed, userFiles, projectYamlPath, formData, userMetadata, }) => {
    return schemaInfo.map((table) => {
        const replacements = getReplacementsForTable(table, schemaInfoParsed);
        if (typeof folderName !== 'string') {
            throw new Error('Folder name is not a string');
        }
        const processedName = replacePlaceholders(folderName, replacements, userFiles, schemaInfoParsed, table, projectYamlPath, undefined, formData, userMetadata);
