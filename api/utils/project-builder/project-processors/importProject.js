 ...
 */
export const importProject = ({ command, schemaInfo, schemaInfoParsed, userFiles, projectYamlPath, table, formData, userMetadata, }) => {
    ...
    try {
        ...
        if (table && scopedOption) {
        ...
        else if ((includeTableOption != null && includeTableOption.trim().length > 0) ||
            ...
            for (const currentTable of schemaInfo) {
                ...
                if (includeTableOption != null &&
                    includeTableOption.trim().length > 0) {
                    const processedIncludeTable = replacePlaceholders(String(options[ACTION_FLAGS.INCLUDE_TABLE]), replacements, userFiles, schemaInfoParsed, currentTable, projectYamlPath, undefined, formData, userMetadata);
                    if (currentTable.tableName !== processedIncludeTable) {
                        continue;
                    }
                }
                // Check exclude filter
                if (excludeTableOption != null &&
                    excludeTableOption.trim().length > 0) {
                    const processedExcludeTable = replacePlaceholders(String(options[ACTION_FLAGS.EXCLUDE_TABLE]), replacements, userFiles, schemaInfoParsed, currentTable, projectYamlPath, undefined, formData, userMetadata);
