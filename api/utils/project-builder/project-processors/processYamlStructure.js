...
export const processYamlStructure = ({ node, schemaInfo, schemaInfoParsed, userFiles, projectYamlPath, table, formData, userMetadata, }) => {
    if (typeof node === 'string') {
        ...
        const hasRelativeTemplatePath = ACTION_FLAGS.IS_RELATIVE_PATH in options &&
        ...
        if (hasRelativeTemplatePath) {
            templatePath = processTemplatePathWithFlag(templatePath, projectYamlPath, true);
        }
        if (node.startsWith(`${PROJECT_ACTIONS.CREATE_BASE_METHOD_FILE}(`)) {
            ...
            return createBaseMethodFile(extractedParams, userFiles, projectYamlPath, schemaInfo, schemaInfoParsed, table);
        }
        if (node.startsWith(`${PROJECT_ACTIONS.CREATE_FILE}(`)) {
            ...
            if (table && (scopedOption ?? false)) {
                ...
                const processedName = replacePlaceholders(command, replacements, userFiles, schemaInfoParsed, table, projectYamlPath, command, formData, userMetadata);
                ...
                const outputFileName = processedName.includes('/')
                    ...
                    : processedName;
                let content = '';
                content = replacePlaceholders(processLoopTables(templateContent, schemaInfo, schemaInfoParsed, userFiles, formData, userMetadata), replacements, userFiles, schemaInfoParsed, table, projectYamlPath, typeof templatePath === 'string' && templatePath.length > 0
                    ? templatePath
                    : command, formData, userMetadata);
                content = processIterateInTemplate(content, schemaInfo, schemaInfoParsed, userFiles, table, formData, userMetadata);
            ...
            const processedName = replacePlaceholders(command, replacements, userFiles, schemaInfoParsed, table, projectYamlPath, undefined, formData, userMetadata);
            // Extract just the filename portion if it contains slashes
            const outputFileName = processedName.includes('/')
                ? extractFileNameFromPath(processedName)
                : processedName.replace(/[()]/g, '');
            // Load and process template content
            const templateContent = loadTemplateContent(userFiles, templatePath ?? processedName, projectYamlPath);
            // Process the template with all replacements
            let processedContent = replacePlaceholders(processLoopTables(templateContent, schemaInfo, schemaInfoParsed, userFiles, formData, userMetadata), replacements, userFiles, schemaInfoParsed, schemaInfoProcessed, projectYamlPath, node, formData, userMetadata);
            processedContent = processIterateInTemplate(processedContent, schemaInfo, schemaInfoParsed, userFiles, schemaInfoProcessed, formData, userMetadata);
        ...
        if (templateContent.length > 0) {
            ...
            let processedContent = replacePlaceholders(processLoopTables(templateContent, schemaInfo, schemaInfoParsed, userFiles, formData, userMetadata), replacements, userFiles, schemaInfoParsed, schemaInfoProcessed, projectYamlPath, undefined, formData, userMetadata);
            processedContent = processIterateInTemplate(processedContent, schemaInfo, schemaInfoParsed, userFiles, table, formData, userMetadata);
            // Format the final content with proper character replacements
            const finalContent = formatFileContent(processedContent);
    ...
    if (Array.isArray(node)) {
        return node.flatMap((item) => {
            if (typeof item === 'object' && item !== null && !Array.isArray(item)) {
                ...
                const isRecordWithDynamicFolder = (obj) => typeof obj === 'object' && obj !== null && !Array.isArray(obj);
                if (isRecordWithDynamicFolder(item)) {
                    ...
                    if (keys.length > 0 &&
                        ...
                        const folderName = key.slice(PROJECT_ACTIONS.FOLDER_LOOP.length + 1, -1);
    ...
    if (typeof node === 'object' && node !== null) {
        return Object.entries(node).flatMap(([key, value]) => {
            ...
            if (key.startsWith(`${PROJECT_ACTIONS.IMPORT_PROJECT}(`)) {
                ...
                const commandString = key.slice(PROJECT_ACTIONS.IMPORT_PROJECT.length + 1, key.length - (key.endsWith(':') ? 2 : 1));
                // Process the import
