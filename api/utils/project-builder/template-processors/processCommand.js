...
import { TEMPLATE_ACTIONS, USE_FORM_DATA_REGEX, USE_USER_ENV_REGEX, } from '../../../utils/project-builder/constants/templateActions';
...
export const processCommand = (text, userFiles, schemaInfoParsed, table, templateFilePath, projectFilePath, formData, userMetadata) => {
    // Process all commands in order of specificity
    let result = text;
    // First, process USE_CONSTANT commands
    const useConstantRegex = new RegExp(`\\[\\[\\s*${TEMPLATE_ACTIONS.USE_CONSTANT}\\(([^)]+)\\)\\s*\\]\\]`, 'g');
    result = result.replace(useConstantRegex, (_match, group1) => {
        if (!table) {
            return '';
        }
        const constantName = group1.trim();
        return loadConstant(constantName, userFiles, schemaInfoParsed, table, projectFilePath, formData).join(',');
    });
    ...
    result = processUseTemplate(result, userFiles, schemaInfoParsed, templateFilePath, projectFilePath, table, formData, userMetadata);
    ...
    const lineWithLoopRegex = new RegExp(`^.*?\\[\\[\\s*${TEMPLATE_ACTIONS.LOOP}\\([^)]*?\\).*?\\]\\].*?(\r?\n)?`, 'gm');
    result = result.replace(lineWithLoopRegex, (fullLine, _newlineChar) => {
        ...
        const loopTagRegex = new RegExp(`\\[\\[\\s*${TEMPLATE_ACTIONS.LOOP}\\(([^)]*?)\\)(.*?)\\]\\]`, 'g');
        // We'll use this to track if any of the LOOP commands in the line produced content
        let lineHasContent = false;
        // Process each LOOP tag in the line
        const processedLine = fullLine.replace(loopTagRegex, (fullMatch, group1, group2) => {
            const whitespace = /^\s*/.exec(fullMatch)?.[0] ?? '';
            const propertyPaths = group1;
            const options = group2;
            const cmdResult = processIterateCommand(`${TEMPLATE_ACTIONS.LOOP}(${propertyPaths})${options}`, table, schemaInfoParsed, userFiles, projectFilePath, formData, userMetadata);
    ...
    const iterateRegex = new RegExp(`\\[\\[\\s*${TEMPLATE_ACTIONS.LOOP}\\(([^)]*?)\\)(.*?)\\]\\]`, 'g');
