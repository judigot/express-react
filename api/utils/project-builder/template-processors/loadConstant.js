...
export const loadConstant = (constantName, userFiles, schemaInfoParsed, table, projectFilePath, formData, userMetadata) => {
    const store = userFiles;
    const constantsFolder = store.find((item) => item.type === 'folder' && item.name === 'Constants');
    if (!constantsFolder) {
        return [];
    }
    const constantFile = constantsFolder.children.find((item) => item.type === 'file' && item.name === `${constantName}.yaml`);
    if (!constantFile) {
        return [];
    }
    try {
        // Preprocess content to quote values with curly braces to ensure they're parsed as strings
        const preprocessedContent = constantFile.content.replace(/^-\s*(\{\{[^}]+\}\})\s*$/gm, '- "$1"');
        // Parse YAML content
        ...
        if (table) {
            ...
            return rawValues.map((value) => replacePlaceholders(value, replacements, userFiles, schemaInfoParsed, table, projectFilePath, constantName, formData, userMetadata));
        }
        return rawValues;
    }
    catch {
