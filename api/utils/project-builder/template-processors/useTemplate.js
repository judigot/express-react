...
import { isPathRelative, getProjectDirectory, } from '../../../utils/project-builder/utils/processRelativePath';
/**
 ...
 */
export const processUseTemplate = (text, userFiles, schemaInfoParsed, templatePath, projectFilePath, table, formData, userMetadata) => {
    ...
    return text.replace(globalUseTemplateRegex, (_match, importPath) => {
        ...
        const templateContent = loadTemplateContent(userFiles, pathToUse, projectFilePath);
        ...
        const processedContent = processCommand(templateContent, userFiles, schemaInfoParsed, table, 
        // Use the resolved path to ensure proper relative path resolution for nested imports
        pathToUse, projectFilePath, formData, userMetadata);
