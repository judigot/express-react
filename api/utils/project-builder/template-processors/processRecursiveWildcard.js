...
export const findFoldersWithWildcard = (root, pathPattern) => {
    ...
    while (segmentIndex < pathSegments.length) {
        ...
        folders = matchingFolders.flatMap((folder) => folder.children.filter((item) => item.type === 'folder'));
        segmentIndex++;
...
export const getAllDescendantFolders = (folders) => {
    ...
    while (queue.length > 0) {
        ...
        const childFolders = current.children.filter((item) => item.type === 'folder');
 ...
 */
export const getAllDescendantFoldersMatchingPath = (rootFolders, remainingPath) => {
    const result = [];
...
export const buildFolderPath = (targetFolder, root) => {
    ...
    const rootLevelFolder = root.find((item) => item.type === 'folder' && item === targetFolder);
    if (rootLevelFolder) {
