 ...
 */
export const importTemplateAsPlaceholder = (template, replacements, processedKeys = new Set(), path = '') => {
 ...
 */
const processStringPlaceholders = (template, replacements, processedKeys, path) => {
    ...
    return template.replace(/\{\{([^}]+)\}\}/g, (_match, placeholder) => {
        ...
        if (typeof value === 'string' && value.includes('{{')) {
            ...
            processedValue = value.map((item) => typeof item === 'string' && item.includes('{{')
