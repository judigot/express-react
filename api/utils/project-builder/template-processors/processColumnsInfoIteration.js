...
export const processColumnsInfoIteration = (tableObj, templateStr, separatorStr, schemaInfoParsed, userFiles, projectFilePath, formData, userMetadata) => {
    ...
    for (const column of tableObj.columnsInfo) {
        ...
        const result = replacePlaceholders(processedTemplate, replacements, userFiles, schemaInfoParsed, tableObj, projectFilePath, processedTemplate, formData, userMetadata);
        if (result.trim()) {
            results.push(result);
        }
    }
