...
import { LOOP_COMMAND_REGEX, LOOP_TABLES_REGEX, TEMPLATE_MATCH_REGEX, SEPARATOR_MATCH_REGEX, FILTER_MATCH_REGEX, IGNORE_MATCH_REGEX, INCLUDE_FILES_MATCH_REGEX, EXCLUDE_FILES_MATCH_REGEX, REMOVE_DUPLICATES_REGEX, USE_CONSTANT_REGEX, FOLDER_PATH_REGEX, RECURSIVE_WILDCARD_REGEX, } from '../../../utils/project-builder/constants/templateActions';
...
import { findFoldersWithWildcard, buildFolderPath, } from '../../../utils/project-builder/template-processors/processRecursiveWildcard';
...
const findFolderByPath = (path, userFiles) => {
    ...
    currentFolder = store.find((item) => item.type === 'folder' && item.name === pathParts[0]);
    // Navigate through the rest of the path
    for (let i = 1; i < pathParts.length && currentFolder; i++) {
        currentFolder = currentFolder.children.find((item) => item.type === 'folder' && item.name === pathParts[i]);
    }
...
const folderContainsYamlFiles = (folder) => {
    return folder.children.some((item) => item.type === 'file' &&
        (item.name.endsWith('.yml') || item.name.endsWith('.yaml')));
};
export const processLoopTables = (content, schemaInfo, schemaInfoParsed, userFiles, formData, userMetadata) => {
    return content.replace(LOOP_TABLES_REGEX, (_match, options) => {
        ...
        return schemaInfo
            ...
            return replacePlaceholders(templateContent.trim(), replacements, userFiles, schemaInfoParsed, table, undefined, undefined, formData, userMetadata);
        })
            .join(separator);
    });
};
export const processIterateCommand = (command, table, schemaInfoParsed, userFiles, projectFilePath, formData, userMetadata) => {
    ...
    const ignoreList = ignoreMatch
        ? ignoreMatch[1].split(',').flatMap((item) => {
            ...
            if (constantMatch) {
                ...
                return loadConstant(constantMatch[1], userFiles, schemaInfoParsed, table, undefined, formData, userMetadata);
            }
            // For non-constant values, still process any placeholders they might have
            const replacements = getReplacementsForTable(table, schemaInfoParsed);
            return replacePlaceholders(trimmed, replacements, userFiles, schemaInfoParsed, table, undefined, undefined, formData, userMetadata);
        })
        : [];
    ...
    if (recursiveWildcardMatch) {
        ...
        if (shouldBeFileBased) {
            ...
            for (const folder of filteredFolders) {
                ...
                const fullPath = buildFolderPath(folder, userFiles);
                const processedTemplate = processFileBasedTemplate(fullPath, userFiles, schemaInfoParsed, table, template, includedFiles, excludedFiles, separator, projectFilePath, formData, userMetadata);
    ...
    if (folderPathMatch) {
        ...
        if (shouldBeFileBased) {
            return processFileBasedTemplate(propertyPathsStr, userFiles, schemaInfoParsed, table, template, includedFiles, excludedFiles, separator, projectFilePath, formData, userMetadata);
        }
    }
    ...
    const filterList = filterMatch
        ? filterMatch[1].split(',').map((item) => {
            ...
            if (constantMatch) {
                ...
                return loadConstant(constantMatch[1], userFiles, schemaInfoParsed, table);
            }
            // For non-constant values, still process any placeholders they might have
            const replacements = getReplacementsForTable(table, schemaInfoParsed);
            return replacePlaceholders(trimmed, replacements, userFiles, schemaInfoParsed, table, undefined, undefined, formData, userMetadata);
    ...
    if (propertyPaths.includes('columnsInfo')) {
        return processColumnsInfoIteration(table, template, separator, schemaInfoParsed, userFiles, projectFilePath, formData);
    }
    ...
    for (const path of propertyPaths) {
        ...
        if (folderMatch) {
            ...
            if (recursiveWildcardMatch && path === propertyPathsStr) {
                ...
                for (const folder of filteredFolders) {
                    processFilesInFolder(folder, includedFiles, excludedFiles, allValues, allPlaceholderValues, fileNameMap);
                }
            ...
            if (currentFolder) {
                ...
                processFilesInFolder(currentFolder, includedFiles, excludedFiles, allValues, allPlaceholderValues, fileNameMap);
            }
        }
    }
    for (const propertyPath of propertyPaths) {
        ...
        switch (cleanPath) {
            ...
            case 'pivotRelationships.relatedTable': {
                const pivotTables = table.pivotRelationships?.map((rel) => rel.relatedTable) ?? [];
                allValues.push(...pivotTables);
                break;
            }
            case 'pivotRelationships.pivotTable': {
                const pivotTables = table.pivotRelationships?.map((rel) => rel.pivotTable) ?? [];
                allValues.push(...pivotTables);
                break;
            }
            case 'columnsInfo': {
    ...
    const lines = finalValues.map((value) => {
        ...
        return replacePlaceholders(template, replacements, userFiles, schemaInfoParsed, table, projectFilePath, template, formData, userMetadata);
    });
 ...
 */
const processFilesInFolder = (folder, includedFiles, excludedFiles, allValues, allPlaceholderValues, fileNameMap) => {
    folder.children.forEach((item) => {
        if (item.type === 'file') {
            ...
            const shouldExclude = excludedFiles.some((pattern) => fileName.includes(pattern));
