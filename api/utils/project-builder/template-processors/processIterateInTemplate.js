...
export const processIterateInTemplate = (content, schemaInfo, schemaInfoParsed, userFiles, table, formData, userMetadata) => {
    const iterateRegex = new RegExp(`\\[\\[\\s*${TEMPLATE_ACTIONS.LOOP}\\(([^\\[\\]]*?(?:\\{\\{[^}]*\\}\\})?[^\\[\\]]*)\\)([^\\]]*)\\]\\]`, 'g');
    return content.replace(iterateRegex, (fullMatch, propertyPathsStr, options) => {
        ...
        if (table) {
            ...
            const cmdResult = processIterateCommand(`${TEMPLATE_ACTIONS.LOOP}(${propertyPathsStr})${options}`, table, schemaInfoParsed, userFiles, undefined, formData, userMetadata);
