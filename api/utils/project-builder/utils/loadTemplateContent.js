...
const navigateToFile = (store, filePath) => {
    ...
    const file = currentItems.find((item) => item.type === 'file' && item.name === fileName);
    return file ?? null;
};
export const loadTemplateContent = (userFiles, templatePath, projectFilePath) => {
...
const processTemplateContent = (content) => {
    function replaceOutsideTemplateSeparator(input) {
        ...
        protectedString = protectedString.replace(/__PLACEHOLDER_(\d+)__/g, (_, p1) => {
            const i = parseInt(p1, 10);
            return placeholders[i] ?? '';
        });
        return protectedString;
    }
    return replaceOutsideTemplateSeparator(content
