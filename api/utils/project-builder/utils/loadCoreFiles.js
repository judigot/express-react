...
const findProjectFolder = (projectYamlPath, userFiles) => {
    const normPath = projectYamlPath.startsWith('/')
        ...
        : projectYamlPath;
    const projectsFolder = userFiles.find((item) => item.type === 'folder' && item.name === 'Projects');
    if (!projectsFolder) {
        return null;
    }
    for (const child of projectsFolder.children) {
        if (child.type === 'folder') {
            const structureFile = child.children.find((file) => file.type === 'file' && file.name === 'structure.yaml');
            if (structureFile) {
...
const resolveCoreImport = (corePath, userFiles) => {
    ...
    for (const component of pathComponents) {
        const folder = currentItems.find((item) => item.type === 'folder' && item.name === component);
        if (!folder) {
            return [];
        }
...
export const loadCoreFiles = (projectYamlPath, userFiles) => {
    ...
    const structureFile = projectFolder.children.find((file) => file.type === 'file' && file.name === 'structure.yaml');
    let mergedCores = [];
    if (structureFile) {
        try {
    ...
    const localCoreFolder = projectFolder.children.find((item) => item.type === 'folder' && item.name === 'core');
    if (localCoreFolder) {
        mergedCores = mergeCoreFilesWithScaffolded(mergedCores, localCoreFolder.children);
    }
    return mergedCores;
};
