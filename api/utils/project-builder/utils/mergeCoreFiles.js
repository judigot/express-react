...
export const mergeCoreFilesWithScaffolded = (coreFiles, scaffoldedFiles) => {
    ...
    for (const scaffoldedItem of scaffoldedFiles) {
        const existingIndex = merged.findIndex((item) => item.name === scaffoldedItem.name && item.type === scaffoldedItem.type);
        if (existingIndex === -1) {
            ...
            if (scaffoldedItem.type === 'folder' && existingItem.type === 'folder') {
                const mergedFolder = {
                    ...
                    children: mergeCoreFilesWithScaffolded(existingItem.children, scaffoldedItem.children),
                };
