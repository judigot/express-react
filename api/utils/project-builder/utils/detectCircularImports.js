...
import { IMPORT_PROJECT_REGEX, PROJECT_ACTIONS, } from '../../../utils/project-builder/constants/projectActions';
...
function extractImportPaths(content) {
    ...
    if (isRecord(parsedYaml)) {
        ...
        const rootImportKeys = Object.keys(parsedYaml).filter((key) => key.startsWith(PROJECT_ACTIONS.IMPORT_PROJECT));
 ...
 */
export const detectCircularImports = (filePath, userFiles, visited = new Set(), recStack = []) => {
    ...
    if (file && typeof file.content === 'string' && file.content.length > 0) {
        ...
        for (const importPath of importPaths) {
            const result = detectCircularImports(importPath, userFiles, visited, recStack);
