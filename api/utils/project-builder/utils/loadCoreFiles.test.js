...
describe('loadCoreFiles', () => {
    ...
    it('should load single core import', () => {
        ...
        const result = loadCoreFiles('/Projects/Test Project/structure.yaml', userFiles);
    ...
    it('should merge multiple core imports in order', () => {
        const userFiles = createMockStructure();
        const result = loadCoreFiles('/Projects/Multi Import Project/structure.yaml', userFiles);
        expect(result.find((f) => f.name === '.gitignore')).toBeDefined();
        expect(result.find((f) => f.name === 'package.json')).toBeDefined();
    ...
    it('should handle project with no core imports', () => {
        const userFiles = createMockStructure();
        const result = loadCoreFiles('/Projects/No Core Project/structure.yaml', userFiles);
        expect(result).toEqual([]);
    });
    it('should load local core folder when no imports specified', () => {
        const userFiles = createMockStructure();
        const result = loadCoreFiles('/Projects/Local Core Only/structure.yaml', userFiles);
        expect(result).toHaveLength(1);
        expect(result.find((f) => f.name === 'config.js')).toBeDefined();
    });
    it('should prioritize local core over imports', () => {
        const userFilesWithConflict = [
            {
                type: 'folder',
                name: 'Core',
                children: [
        ...
        const result = loadCoreFiles('/Projects/Override Project/structure.yaml', userFilesWithConflict);
    ...
    it('should return empty array for non-existent project', () => {
        const userFiles = createMockStructure();
        const result = loadCoreFiles('/Projects/Non Existent/structure.yaml', userFiles);
        expect(result).toEqual([]);
    });
    it('should handle invalid core import paths gracefully', () => {
        ...
        const result = loadCoreFiles('/Projects/Bad Import/structure.yaml', userFiles);
        expect(result).toEqual([]);
    });
});
