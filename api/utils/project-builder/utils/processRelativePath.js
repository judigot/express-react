...
export const processRelativePath = (path, projectFilePath) => {
    ...
    const absolutePath = projectDir.length > 0 ? `${projectDir}/${normalizedPath}` : normalizedPath;
    return absolutePath;
};
/**
 ...
 */
export const processTemplatePathWithFlag = (templatePath, projectYamlPath, isRelativeFlag) => {
    ...
    const projectDirectory = projectYamlPath.replace(extractFileNameFromPath(projectYamlPath), '');
