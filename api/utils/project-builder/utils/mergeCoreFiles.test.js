...
describe('mergeCoreFilesWithScaffolded', () => {
    ...
    it('should merge folders with same name', () => {
        ...
        if (result[0].type === 'folder') {
            ...
            expect(result[0].children.find((f) => f.name === 'core-file.txt')).toBeDefined();
            expect(result[0].children.find((f) => f.name === 'scaffolded-file.txt')).toBeDefined();
        }
    });
    it('should deeply merge nested folders', () => {
        ...
        if (result[0].type === 'folder') {
            ...
            if (utilsFolder?.type === 'folder') {
                ...
                expect(utilsFolder.children.find((f) => f.name === 'core-helper.js')).toBeDefined();
                expect(utilsFolder.children.find((f) => f.name === 'scaffolded-helper.js')).toBeDefined();
            }
            const componentsFolder = result[0].children.find((f) => f.name === 'components');
    ...
    it('should override file in nested folder', () => {
        ...
        if (result[0].type === 'folder') {
            ...
            if (result[0].children[0].type === 'file') {
                expect(result[0].children[0].content).toBe('Scaffolded database config');
            }
        }
    });
    ...
    it('should handle complex real-world scenario', () => {
        ...
        if (srcFolder?.type === 'folder') {
            expect(srcFolder.children.find((f) => f.name === 'test.txt')).toBeDefined();
            expect(srcFolder.children.find((f) => f.name === 'components')).toBeDefined();
            ...
            if (configFolder?.type === 'folder') {
                ...
                expect(configFolder.children.find((f) => f.name === 'app.js')).toBeDefined();
                expect(configFolder.children.find((f) => f.name === 'routes.js')).toBeDefined();
            }
        }
    });
});
