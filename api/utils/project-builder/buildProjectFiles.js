...
export const buildProjectFiles = (projectYamlPath, userFiles, schemaInfo, formData, userMetadata) => {
    ...
    try {
        ...
        if (parsedYaml !== null &&
            ...
            const entries = Object.entries(parsedYaml).filter(([key]) => key !== '$USE_CORE');
        ...
        const projectFiles = mergeCoreFilesWithScaffolded(coreFiles, scaffoldedFiles);
        return projectFiles.filter((item) => item.name !== 'core' && item.name !== 'Core');
    }
    catch (error) {
        return [
            {
