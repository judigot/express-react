...
export const getColumnDefaultDisplay = ({ isPrimaryKey, isNullable, columnDefault, }) => {
...
export function getPrimaryKey({ tableName, schemaInfo, }) {
    const tableSchema = schemaInfo.find((schema) => schema.tableName === tableName);
    if (!tableSchema) {
        throw new Error(`Table "${tableName}" not found in schema information.`);
    }
    const primaryKeyColumn = tableSchema.columnsInfo.find((column) => Boolean(column.primary_key));
    if (!primaryKeyColumn) {
...
export const generateModelImports = (schemaInfo) => {
    ...
    columnsInfo.forEach((column) => {
        if (column.foreign_key) {
            const relatedClass = changeCase(column.foreign_key.foreign_table_name).pascalCase;
            imports.add(`use App\\Models\\${relatedClass};`);
        }
    });
...
const getTypeMappingValue = (mappings, key, columnType) => {
    if (Object.hasOwn(mappings, key) && isRecord(mappings[key])) {
        ...
        if (Object.hasOwn(mapping, columnType) &&
...
export const generateColumnDefinition = ({ columnName, columnType, }) => {
    ...
    const isDBDefinition = dbTypes !== undefined &&
        dbTypes.length > 0 &&
        typeof columnType === 'string' &&
        dbTypes.includes(columnType);
    const targetDefinition = columnMappings[isDBDefinition ? 'sql-table' : 'typescript'];
