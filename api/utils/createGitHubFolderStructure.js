...
async function createBlobs(octokit, owner, repo, files) {
    ...
    const createBlobWithRetry = async (file, attempt = 0) => {
        try {
            const base64Content = file.isBinary === true
                ? file.content
            ...
            if (error instanceof Error) {
                ...
                if (isRateLimit && attempt < GITHUB_BLOB_MAX_RETRIES) {
                    const backoffMs = GITHUB_BLOB_RETRY_BASE_DELAY_MS * 2 ** attempt +
...
export const createGitHubFolderStructure = async (data) => {
    const { structure, owner, repo, githubToken, basePath = '', branch = 'main', commitMessage, projectName, } = data;
    ...
    try {
        ...
        const displayName = typeof projectName === 'string' && projectName !== ''
            ? projectName
            : repo;
...
🚀 Write Once. Generate Forever.`;
        ...
        const projectFilesCommitMessage = commitMessage !== undefined && commitMessage !== ''
            ? commitMessage
