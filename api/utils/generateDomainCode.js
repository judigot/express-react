...
function generateDomainCode({ schemaInfo, tableInfo, tableName, codeToGenerate, relationshipType, }) {
    const determineRelationshipType = ({ relationshipType, tableInfo, relatedTable, }) => {
        ...
        const isPivotRelationship = tableInfo.pivotRelationships?.some((rel) => rel.relatedTable === relatedTable);
        // Find the related table's schema info
        const relatedTableInfo = schemaInfo.find((table) => table.tableName === relatedTable);
        ...
        const isInRelatedTablePivot = relatedTableInfo?.pivotRelationships?.some((rel) => rel.relatedTable === tableName) ?? false;
        return {
            belongsTo,
            hasOne,
            hasMany,
            ...
            isOneToMany: (hasMany && isInRelatedTableBelongsTo) ||
    ...
    const primaryKey = tableInfo.columnsInfo.find((column) => column.primary_key ?? false)?.column_name;
    if (primaryKey == null) {
        throw new Error(`Primary key not found for table "${tableName}".`);
    }
    const { hasOne, hasMany, pivotRelationships, isPivot, belongsTo, belongsToMany, } = tableInfo;
    ...
    const generateCodeFromTable = (relatedTable, relationshipType) => {
        ...
        const pivotTableName = relationshipType === 'belongsToMany' || relationshipType === 'manyToMany'
            ? schemaInfo.find((table) => Boolean(table.foreignTables &&
                table.foreignTables.includes(relatedTable) &&
                table.foreignTables.includes(tableName)))?.tableName
            : tableInfo.pivotRelationships?.find((rel) => rel.relatedTable === relatedTable)?.pivotTable;
        ...
        if (relatedTableForeignKey == null) {
            throw new Error(`ForeignKey not found for table "${tableName}" and related table "${relatedTable}".`);
        }
        ...
        const { singular: tableNameSingular, pascalCase: tableNamePascalCase, kebabCase: tableNameKebab, kebabCasePlural: tableNameKebabCasePlural, } = changeCase(tableName);
        const { plural: relatedTableNamePlural, pascalCase: relatedTableNamePascal, pascalCasePlural: relatedTableNamePascalPlural, kebabCase: relatedTableNameKebabCase, kebabCasePlural: relatedTableNameKebabCasePlural, } = changeCase(relatedTable);
        const { pascalCase: pivotTableNamePascal, kebabCase: pivotTableNameKebabCase, } = pivotTableName !== undefined
        ...
        const hasPivotRelationships = pivotRelationships && pivotRelationships.length > 0;
        ...
        return rawMethods
            ...
            const repositoryMethod = typeof method.repositoryMethod === 'function'
                ? method.repositoryMethod(status)
                : method.repositoryMethod;
            const controllerMethodName = typeof method.controllerMethod === 'function'
    ...
    const allMethods = [
        ...
        ...(belongsToMany ?? []).flatMap((table) => generateCodeFromTable(table, 'belongsToMany')),
        ...(pivotRelationships ?? []).flatMap(({ relatedTable }) => generateCodeFromTable(relatedTable, 'manyToMany')),
    ];
