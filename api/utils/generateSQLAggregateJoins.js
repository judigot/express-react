    ...
    const joinQueries = schemaInfo
        ...
        return hasMany?.map((childTable) => {
            ...
            const childForeignKey = schemaInfo
                ...
                ?.columnsInfo.find((col) => col.foreign_key?.foreign_table_name === tableName)?.column_name;
            if (childForeignKey == null) {
                throw new Error(`Foreign key not found for table ${childTable} referencing ${tableName}`);
            }
