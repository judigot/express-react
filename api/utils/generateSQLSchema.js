...
import { determineSQLDatabaseType, generateColumnDefinition, } from '../utils/common';
...
const generateSQLSchema = (schemaInfo) => {
    ...
    const generateForeignKeyConstraint = (tableName, schemaInfo) => {
        ...
        return tableInfo.columnsInfo
            ...
            if (APP_SETTINGS.onDeleteCascade) {
                const parentTable = schemaInfo.find((t) => t.tableName === fk.foreign_table_name);
    ...
    return formatSQL(schemaInfo
        ...
        const columns = columnsInfo
            .map((column) => generateColumnDefinition({
            columnName: column,
            columnType: dbType,
        }))
            .join(',\n  ');
        const foreignKeyConstraints = generateForeignKeyConstraint(tableName, schemaInfo).join(',\n  ');
