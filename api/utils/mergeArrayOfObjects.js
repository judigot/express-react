export function mergeArrayOfObjects(structure1, structure2, groupBy) {
    ...
    for (const item of [...structure1, ...structure2]) {
        const existingItemIndex = mergedArray.findIndex((existing) => areObjectsSimilar(existing, item));
        if (existingItemIndex !== -1) {
            // Merge similar objects
            mergedArray[existingItemIndex] = mergeObjects(mergedArray[existingItemIndex], item);
        }
        else {
            // Add new object
