...
import { getForeignKeys, getForeignTables, getRequiredColumns, } from '../utils/convertIntrospectedStructure';
...
export const isJunctionTable = (relationship, schemaInfo) => {
    const foreignKeys = relationship.columnsInfo.filter((column) => Boolean(column.foreign_key));
    if (foreignKeys.length === 2) {
        const [firstKey, secondKey] = foreignKeys;
        // Find the parent tables
        const parentTable1 = schemaInfo.find((rel) => rel.tableName === firstKey.foreign_key?.foreign_table_name);
        const parentTable2 = schemaInfo.find((rel) => rel.tableName === secondKey.foreign_key?.foreign_table_name);
        const hasNoChildTables = (relationship.childTables ?? []).length === 0;
        if (parentTable1 && parentTable2) {
            const isNameCombination = relationship.tableName ===
...
export const addAssociations = (schemaInfo, data = null) => {
    ...
    const handleBelongsToMany = (relationship) => {
        const foreignKeys = relationship.columnsInfo.filter((col) => Boolean(col.foreign_key));
        if (foreignKeys.length === 2) {
            const table1 = tempSchemaInfo.find((rel) => rel.tableName === foreignKeys[0].foreign_key?.foreign_table_name);
            const table2 = tempSchemaInfo.find((rel) => rel.tableName === foreignKeys[1].foreign_key?.foreign_table_name);
            if (table1 && table2) {
    ...
    tempSchemaInfo.forEach((relationship) => {
        ...
        relationship.columnsInfo.forEach((column) => {
            if (column.foreign_key) {
                const parentTable = tempSchemaInfo.find((rel) => rel.tableName === column.foreign_key?.foreign_table_name);
                if (parentTable) {
                    // --- belongsTo ---
                    ...
                    if (!isIntrospection) {
                        ...
                        const foreignTable = tempSchemaInfo.find((rel) => rel.columnsInfo.some((col) => Boolean(col.primary_key && col.column_name === column.column_name)));
                        if (foreignTable) {
                            if (!column.unique) {
                                ...
                                if (!foreignTable.hasMany) {
                                    ...
                                    foreignTable.hasMany = Array.from(new Set(foreignTable.hasMany));
                                }
                            }
                            else if (!isJunctionTable(relationship, schemaInfo)) {
                                // --- foreignTable.hasOne ---
                                if (!foreignTable.hasOne) {
                                    ...
                                    foreignTable.hasOne = Array.from(new Set(foreignTable.hasOne));
                                }
                            }
                        }
                    }
...
*/
export function createColumnsInfo({ fields, rows, tableName, primaryKeyField, }) {
    return Object.keys(fields).map((key) => {
        ...
        const foreignKey = key.endsWith('_id') && key !== `${tableName}_id`
            ? {
    ...
    return schemaInfo.map((relationship) => {
        if (relationship.foreignTables) {
            relationship.foreignTables.forEach((foreignTable) => {
                const foreignRelationship = schemaInfo.find((r) => r.tableName === foreignTable);
    ...
    schemaInfo.forEach((info) => {
        if (info.belongsTo != null) {
            info.belongsTo.forEach((parentTable) => {
                const parentTableInfo = schemaInfo.find((schema) => schema.tableName === parentTable);
                if (parentTableInfo) {
                    const partnerTable = info.belongsTo?.find((table) => table !== parentTable);
...
export const determineUniqueForeignKeys = (schemaInfo) => {
    schemaInfo.forEach((relationship) => {
        relationship.columnsInfo.forEach((column) => {
            if (column.foreign_key) {
                const parentTable = schemaInfo.find((rel) => rel.tableName === column.foreign_key?.foreign_table_name);
                if (parentTable) {
                    const isOneToOne = parentTable.hasOne?.includes(relationship.tableName);
                    if (isOneToOne === true) {
                        column.unique = true;
                    }
                }
...
export function identifySchema(data) {
    const schemaInfo = Object.keys(data).map((tableName) => {
        ...
        const primaryKeyField = determinePrimaryKeyField(tableName, Object.keys(rows[0])[0]);
