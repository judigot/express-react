...
const fetchUserMetadata = async (accessToken) => {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to load user metadata: ${String(response.status)} ${response.statusText} ${errorText}`);
    }
...
export const useUser = () => {
    const { user: auth0User, isAuthenticated, isLoading: auth0Loading, getAccessTokenSilently, error: auth0Error, logout: auth0Logout, } = useAuth0();
    const { setUser, setAccessToken, setUserMetadata, setGithubToken, userMetadata, githubToken, } = useUserStore();
