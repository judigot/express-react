...
describe('createModels', () => {
    ...
    it('should generate correct relationships for User model with one-to-one Post', () => {
        const userTableInfo = userPostOneToOneSchemaInfo.find((table) => table.tableName === 'user');
...
class User extends Model
{
    ...
    `);
        const generatedUserModel = template
            ...
            .replace('{{fillable}}', "'first_name',\n        'last_name',\n        'email',\n        'username',\n        'password'")
            .replace('{{relationships}}', userRelationships.join('\n'));
        expect(normalizeWhitespace(generatedUserModel)).toEqual(normalizeWhitespace(expectedUserModel));
    });
    it('should generate correct relationships for User model', () => {
        const userTableInfo = userPostsOneToManySchemaInfo.find((table) => table.tableName === 'user');
        if (userTableInfo == null) {
            throw new Error('User table not found in schema');
        }
        const userRelationships = generateDomainCode({
...
class User extends Model
{
    ...
    it('should generate correct relationships for Post model', () => {
        const postTableInfo = userPostsOneToManySchemaInfo.find((table) => table.tableName === 'post');
...
class Post extends Model
{
    ...
    `);
        const generatedPostModel = template
            ...
            .replace('{{fillable}}', "'user_id',\n        'title',\n        'content'")
            .replace('{{relationships}}', postRelationships.join('\n'));
        expect(normalizeWhitespace(generatedPostModel)).toEqual(normalizeWhitespace(expectedPostModel));
    });
    it('should generate correct relationships for Customer model', () => {
        const customerTableInfo = POSSchemaInfo.find((table) => table.tableName === 'customer');
...
class Customer extends Model
{
    ...
    `);
        ...
        expect(normalizeWhitespace(generatedCustomerModel)).toEqual(normalizeWhitespace(expectedCustomerModel));
    });
    it('should generate correct relationships for Order model', () => {
        const orderTableInfo = POSSchemaInfo.find((table) => table.tableName === 'order');
...
class Order extends Model
{
    ...
    `);
        const generatedOrderModel = template
            .replace('{{ownerComment}}', watermark)
            .replace('{{modelImports}}', 'use App\\Models\\Customer;\nuse App\\Models\\OrderProduct;\nuse App\\Models\\Product;')
        ...
        expect(normalizeWhitespace(generatedOrderModel)).toEqual(normalizeWhitespace(expectedOrderModel));
    });
    it('should generate correct relationships for OrderProduct model', () => {
        const orderProductTableInfo = POSSchemaInfo.find((table) => table.tableName === 'order_product');
...
class OrderProduct extends Model
{
    ...
    `);
        ...
        expect(normalizeWhitespace(generatedOrderProductModel)).toEqual(normalizeWhitespace(expectedOrderProductModel));
    });
});
