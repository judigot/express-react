...
export const useProjectStore = create()(persist((set, get) => ({
    ...
    processFilesUpdate: (_userFiles, projects) => {
        ...
        if (Object.keys(projectBuildCache).length > 0) {
            ...
            const removedProjects = cachedProjectNames.filter((name) => !existingProjectNames.includes(name));
        ...
        if (selectedProject !== null) {
            const stillExists = projects.some((p) => p.name === selectedProject.name);
            if (!stillExists) {
    ...
    buildProjectFilesForProject: (project, schemaInfo) => {
        ...
        const projectPath = project.uniqueId != null && project.uniqueId.length > 0
            ? project.uniqueId
            : `/Projects/${project.name}/structure.yaml`;
        const builtFiles = buildProjectFiles(projectPath, allUserFiles, schemaInfo, formData, userMetadata);
        // Update cache
        set((state) => ({
            projectBuildCache: {
