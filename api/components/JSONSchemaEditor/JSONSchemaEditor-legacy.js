...
const App = () => {
    ...
    const handleAddTable = () => {
        if (newTableName) {
            ...
            const doesTableIdExist = Object.values(schema).some((rows) => rows.some((row) => newTableId in row));
            if (!doesTableIdExist) {
    ...
    const handleSchemaChange = (e) => {
        ...
        */
        ...
        const isTableNameEdited = previousWord in oldSchema && !(previousWord in updatedSchema);
        if (isTableNameEdited) {
            ...
            const updatedSchemaWithoutOldTable = Object.fromEntries(Object.entries(newSchema).filter(([key]) => key !== previousWord));
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "mb-4", children: [_jsx("input", { id: "newTableName", type: "text", placeholder: "Enter table name", value: newTableName, onChange: handleInputChange, className: "mb-4 p-2 h-10 mt-1 block w-full border bg-gray-900 text-white rounded-md shadow-sm focus:ring focus:ring-indigo-500 focus:ring-opacity-50", onKeyDown: (e) => {
                            if (e.key === 'Enter') {
                                handleAddTable();
                                e.preventDefault();
                            }
                        } }), _jsx("button", { onClick: handleAddTable, disabled: !newTableName, className: `px-4 py-2 text-white font-semibold rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50 
    ${newTableName ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-gray-400 cursor-not-allowed'}
    dark:bg-indigo-700 dark:hover:bg-indigo-600 dark:focus:ring-indigo-300 dark:focus:ring-opacity-50 dark:disabled:bg-gray-600`, type: "button", children: "Add Table" })] }), _jsx("textarea", { ref: textAreaRef, rows: 10, className: "p-2 mt-1 block w-full border border-gray-700 bg-gray-900 text-white rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50", value: JSON.stringify(schema, null, 4), onChange: handleSchemaChange, onKeyUp: handleKeyDown }), !isValidJson && (_jsx("p", { className: "mt-2 text-sm text-red-600 dark:text-red-400", children: "Invalid JSON format. Please correct it." }))] }));
};
export default App;
