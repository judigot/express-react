...
import { Close as CloseIcon, ContentCopy as CopyIcon, Code as CodeIcon, Folder as FolderIcon, CreateNewFolder as CreateNewFolderIcon, NoteAdd as NoteAddIcon, Save as SaveIcon, Download as DownloadIcon, } from '@mui/icons-material';
...
function FileViewer({ folderStructure: initialFolderStructure, mode, projectName, }) {
        ...
        const updateFileInStructure = (items, path, fileName, newContent) => {
            ...
            return items.map((item) => {
                if (item.type === 'folder' && item.name === currentFolder) {
                    return {
                        ...item,
                        children: updateFileInStructure(item.children, remainingPath, fileName, newContent),
                    };
                }
                return item;
            });
        };
        const updatedStructure = updateFileInStructure(folderStructure, currentPath, selectedFile.name, newContent);
    ...
    const addNewFile = (fileName, customPath) => {
        ...
        const updatedStructure = addFileToStructure(folderStructure, newFilePath, newFile);
        setFolderStructure(updatedStructure);
    };
    // Add new folder
    const addNewFolder = (folderName, customPath) => {
        ...
        const addFolderToStructure = (items, path, newFolder) => {
            ...
            return items.map((item) => {
                if (item.type === 'folder' && item.name === currentFolder) {
                    return {
                        ...item,
                        children: addFolderToStructure(item.children, remainingPath, newFolder),
                    };
                }
                return item;
            });
        ...
        const updatedStructure = addFolderToStructure(folderStructure, newFolderPath, newFolder);
        setFolderStructure(updatedStructure);
    };
        ...
        const renameItemInStructure = (items, path, itemToRename, newName) => {
            if (path.length === 0) {
                return items.map((currentItem) => {
                    if (currentItem.name === itemToRename.name &&
            ...
            return items.map((currItem) => {
                if (currItem.type === 'folder' && currItem.name === currentFolder) {
                    return {
                        ...currItem,
                        children: renameItemInStructure(currItem.children, remainingPath, itemToRename, newName),
                    };
                }
                return currItem;
            });
        };
        const updatedStructure = renameItemInStructure(folderStructure, parentPath, item, newName);
        setFolderStructure(updatedStructure);
    ...
    const deleteItem = async (item, parentPath = []) => {
        ...
        const deleteItemFromStructure = (items, path, itemName) => {
            ...
            return items.map((currItem) => {
                if (currItem.type === 'folder' && currItem.name === currentFolder) {
                    return {
                        ...currItem,
                        children: deleteItemFromStructure(currItem.children, remainingPath, itemName),
                    };
                }
                return currItem;
            });
        ...
        const updatedStructure = deleteItemFromStructure(folderStructure, parentPath, item.name);
        setFolderStructure(updatedStructure);
    ...
    function renderTree(items, onSelectFile, parentId = '', parentPath = []) {
        ...
        return items.map((item, index) => {
            ...
            // Handle folder items
            if (item.type === 'folder') {
                return (_jsx(TreeItem, { itemId: itemId, label: _jsxs("div", { className: "flex items-center", onContextMenu: (e) => {
                            handleContextMenuWithCheck(e, item, parentPath);
                        }, children: [_jsx(FolderIcon, { fontSize: "small", className: folderColor }), "\u00A0", item.name] }), children: renderTree(item.children, onSelectFile, itemId, [
                        ...parentPath,
                        item.name,
                    ]) }, itemId));
            }
            ...
            return (_jsx(TreeItem, { itemId: itemId, label: _jsxs("div", { className: "flex items-center", onContextMenu: (e) => {
                        handleContextMenuWithCheck(e, item, parentPath);
                    }, children: [_jsx(CodeIcon, { fontSize: "small", className: "text-yellow-500" }), "\u00A0", item.name] }), onClick: () => {
        ...
        void (async () => {
            ...
            try {
                ...
                if (!isTokenResponse(tokenData) ||
                    ...
                    tokenData.token === '') {
                    throw new Error('GitHub token not found. Please set your GitHub token in your profile.');
                }
                const baseRepoName = typeof projectName === 'string' && projectName !== ''
                ...
                const createRepoResponse = await fetch(`${getApiUrl()}/create-github-repository`, {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
                ...
                if (!isCreateRepositoryResponse(createRepoResult) ||
                ...
                const owner = match[1];
                const repo = match[2];
                const uploadResponse = await fetch(`${getApiUrl()}/create-github-folder-structure`, {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
                        Accept: 'application/json',
                ...
                const filesCreated = isUploadResponse(uploadResult) &&
                ...
                openRandomModal({
                    ...
                    content: (_jsxs("div", { className: "space-y-6", children: [_jsx("div", { className: "bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4", children: _jsxs("div", { className: "flex items-center gap-2 mb-2", children: [_jsx("svg", { className: "w-5 h-5 text-green-600 dark:text-green-400", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M5 13l4 4L19 7" }) }), _jsxs("p", { className: "text-green-800 dark:text-green-200 font-semibold", children: ["Successfully exported ", String(filesCreated), " file(s) to GitHub"] })] }) }), _jsxs("div", { className: "space-y-4", children: [_jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2", children: "Clone Repository" }), _jsxs("div", { className: "flex items-center gap-2 bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg p-3", children: [_jsx("code", { className: "flex-1 text-sm text-gray-800 dark:text-gray-200 font-mono break-all", children: cloneCommand }), _jsxs("button", { type: "button", onClick: () => {
                                                            handleCopy(cloneCommand);
                                                        }, className: "flex-shrink-0 px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md transition-colors duration-200 flex items-center gap-2", title: "Copy clone command", children: [_jsx(CopyIcon, { fontSize: "small" }), _jsx("span", { className: "text-sm", children: "Copy" })] })] })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2", children: "Repository" }), _jsxs("a", { href: repoUrl, target: "_blank", rel: "noopener noreferrer", className: "inline-flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-indigo-600 dark:text-indigo-400 rounded-lg transition-colors duration-200 font-medium", children: [_jsx("svg", { className: "w-5 h-5", fill: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { d: "M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" }) }), _jsx("span", { children: "Open Repository" }), _jsx("svg", { className: "w-4 h-4", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" }) })] })] })] }), _jsx("div", { className: "flex justify-end pt-4 border-t border-gray-200 dark:border-gray-700", children: _jsx("button", { type: "button", onClick: () => {
                                    ...
                                    }, className: "px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors duration-200 font-medium", children: "Done" }) })] })),
                });
            }
            catch (error) {
    ...
    return (_jsxs("div", { className: "h-96 p-2", ref: fileViewerRef, children: [_jsxs("div", { className: "flex gap-2 mb-2", children: [_jsx("button", { type: "button", onClick: () => void handleCreateApp(), className: "sm:mr-2 text-xs h-max w-max bg-in px-4 py-2 bg-indigo-600 text-white rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring focus:ring-indigo-500 focus:ring-opacity-50", children: "Create App!" }), _jsx("button", { type: "button", onClick: handleCreateTestFile, disabled: isCreatingFile || !publicRepoURL, className: `text-xs h-max w-max px-4 py-2 rounded-md shadow-sm focus:outline-none focus:ring focus:ring-indigo-500 focus:ring-opacity-50 ${isCreatingFile || !publicRepoURL
                            ? 'bg-gray-600 text-gray-400 cursor-not-allowed'
                            : 'bg-indigo-600 text-white hover:bg-indigo-700'}`, children: isCreatingFile ? 'Creating...' : 'Create Test File' }), _jsx("button", { type: "button", onClick: handleCreateNewTestRepository, disabled: isCreatingRepository || folderStructure.length === 0, className: `text-xs h-max w-max px-4 py-2 rounded-md shadow-sm focus:outline-none focus:ring focus:ring-indigo-500 focus:ring-opacity-50 ${isCreatingRepository || folderStructure.length === 0
                            ? 'bg-gray-600 text-gray-400 cursor-not-allowed'
                            : 'bg-indigo-600 text-white hover:bg-indigo-700'}`, children: isCreatingRepository
                            ? `Exporting ${String(countFiles(folderStructure))} files...`
                            : `Export Into A New Repository (${String(countFiles(folderStructure))} files)` })] }), _jsx("br", {}), _jsxs("div", { className: "grid grid-cols-1 md:grid-cols-3 text-white", children: [_jsx("div", { className: "col-span-1 bg-gray-800 select-none mr-2", children: _jsxs("div", { children: [_jsx("div", { className: "flex justify-between mb-4", children: mode === 'edit' && (_jsxs("div", { className: "flex items-center justify-between w-full", children: [process.env.NODE_ENV === 'development' && (_jsxs(_Fragment, { children: [_jsx("div", { children: _jsx("button", { onClick: () => {
                                                                handleCopy(JSON.stringify(userFiles, null, 4));
                                                            }, className: "sm:mr-2 text-xs h-max w-max bg-in px-4 py-2 bg-indigo-600 text-white rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring focus:ring-indigo-500 focus:ring-opacity-50", children: "Copy User Files" }) }), _jsx("div", { children: _jsx("button", { onClick: () => {
                                                                handleCopy(JSON.stringify(folderStructure, null, 4));
                                                            }, className: "sm:mr-2 text-xs h-max w-max bg-in px-4 py-2 bg-indigo-600 text-white rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring focus:ring-indigo-500 focus:ring-opacity-50", children: "Copy Project Structure" }) })] })), _jsx("button", { onClick: () => {
                                                    zipAndDownloadIStructure(folderStructure, getZipFileName());
                                                }, className: "h-max w-max p-1 text-white rounded-md shadow-sm hover:bg-gray-700 focus:outline-none focus:ring focus:ring-gray-500 focus:ring-opacity-50 flex items-center", title: "Download Project Files", "aria-label": "Download Project Files", children: _jsx(DownloadIcon, { fontSize: "small" }) }), _jsxs("div", { className: "flex space-x-2", children: [_jsx("button", { onClick: () => {
                                                            void (async () => {
                                                                await handleOpenDialog('newFile');
                                                            })();
                                                        }, className: "h-max w-max p-1 text-white rounded-md shadow-sm hover:bg-gray-700 focus:outline-none focus:ring focus:ring-gray-500 focus:ring-opacity-50 flex items-center", title: "New File", "aria-label": "New File", children: _jsx(NoteAddIcon, { fontSize: "small" }) }), _jsx("button", { onClick: () => {
                                                            void (async () => {
                                                                await handleOpenDialog('newFolder');
                                                            })();
                                                        }, className: "h-max w-max p-1 text-white rounded-md shadow-sm hover:bg-gray-700 focus:outline-none focus:ring focus:ring-gray-500 focus:ring-opacity-50 flex items-center", title: "New Folder", "aria-label": "New Folder", children: _jsx(CreateNewFolderIcon, { fontSize: "small" }) })] })] })) }), _jsx("div", { className: "overflow-auto max-h-80", onContextMenu: (e) => {
                                        if (mode === 'edit') {
                                            handleContextMenu(e);
                                        }
                                    }, children: _jsx(SimpleTreeView, { children: renderTree(folderStructure, setSelectedFile) }) })] }) }), _jsx("div", { className: "col-span-1 md:col-span-2", children: selectedFile && (_jsxs("div", { className: "bg-gray-900", children: [_jsxs("div", { className: "mt-2 sticky left-0 top-0 z-20 bg-gray-800 grid grid-cols-[auto_auto] items-center m-0", children: [_jsx("div", { children: _jsxs("div", { className: "bg-[#1f1f1f] w-max p-2 rounded-t-md flex items-center", children: [_jsxs("span", { children: [selectedFile.name, isFileEdited ? ' •' : '', "\u00A0"] }), _jsx("button", { onClick: () => void handleCloseFile(), className: "hover:bg-gray-700 text-white px-1 pb-1 rounded transition-colors duration-150", children: _jsx(CloseIcon, { fontSize: "small" }) })] }) }), _jsxs("div", { children: [_jsx("button", { onClick: () => {
                                                        handleCopy(selectedFile.content);
                                                    }, className: "hover:bg-gray-700 text-white px-2 py-1 rounded float-right", children: _jsx(CopyIcon, { fontSize: "small" }) }), mode === 'edit' && isFileEdited && (_jsx("button", { onClick: saveFileChanges, className: "hover:bg-gray-700 text-white px-2 py-1 rounded float-right transition-all duration-150", children: _jsx(SaveIcon, { fontSize: "small" }) }))] })] }), isImageFile(selectedFile.name) ? (_jsx("div", { className: "flex items-center justify-center bg-gray-900 p-4", style: { height: '20rem' }, children: _jsx("img", { src: `data:image/${selectedFile.name.split('.').pop() ?? 'png'};base64,${selectedFile.content}`, alt: selectedFile.name, className: "max-h-full max-w-full object-contain" }) })) : (_jsx(Editor, { height: "20rem", defaultValue: selectedFile.content, value: fileContent, language: (() => {
                                        ...
                                        fontSize: 14,
                                        lineNumbers: 'on',
                                    }, onChange: handleEditorChange, onMount: handleEditorDidMount }))] })) })] }), contextMenu && (_jsx(ContextMenu, { x: contextMenu.mouseX, y: contextMenu.mouseY, menuItems: (() => {
                    const menuItems = [];
                    ...
                    if (contextMenu.item) {
                        ...
                        if (contextMenu.item.type === 'folder') {
                            menuItems.push({
                                ...
                                onClick: () => {
                                    if (contextMenu.item?.type === 'folder') {
                                        downloadSelectedFolder(contextMenu.item, contextMenu.parentPath ?? []);
                                    }
                                },
                            });
                                ...
                                menuItems.push({
                                    ...
                                    onClick: () => {
                                        if (contextMenu.item?.type === 'folder') {
                                            ...
                                            const findFolderInStructure = (items, path, folderName) => {
                                                if (path.length === 0) {
                                                    const folderCandidate = items.find((item) => item.type === 'folder' &&
                                                ...
                                                for (const item of items) {
                                                    if (item.type === 'folder' &&
                                                        ...
                                                        return findFolderInStructure(item.children, remainingPath, folderName);
                                                    }
                                                }
                                                return null;
                                            };
                                            const folder = findFolderInStructure(folderStructure, contextMenu.parentPath ?? [], contextMenu.item.name);
                                            if (folder) {
