 ...
 * ```
 */
export const ContextMenu = ({ x, y, menuItems, onClose, width = '14rem', appendToBody = false, className = '', }) => {
    ...
    const renderMenu = () => {
        const menu = (_jsx("div", { ref: menuRef, className: `fixed z-50 bg-gray-800 border border-gray-700 rounded-md shadow-lg py-1 text-white ${className}`, style: {
                left: position.left,
                top: position.top,
                width,
            }, role: "menu", "aria-orientation": "vertical", children: menuItems.map((item) => {
                const isDisabled = Boolean(item.disabled);
                return (_jsxs("button", { type: "button", className: `flex items-center px-3 py-2 hover:bg-gray-700 cursor-pointer w-full text-left ${isDisabled ? 'opacity-50 cursor-not-allowed' : ''} ${item.className ?? ''}`, onClick: () => {
                        if (!isDisabled) {
                            onClose();
                            item.onClick();
                        }
                    }, disabled: isDisabled, role: "menuitem", children: [item.icon !== undefined && (_jsx("div", { className: "mr-2 text-yellow-500", children: item.icon })), _jsx("span", { children: item.label })] }, item.id));
            }) }));
        if (appendToBody) {
