...
const DataTypeSelector = ({ value, onChange, onKeyDown, className = '', id, name, required = false, }) => {
    ...
    return (_jsx("div", { className: "relative", children: _jsx("select", { id: id, name: name, value: value, onChange: onChange, onKeyDown: onKeyDown, className: className, required: required, children: Object.entries(groupedOptions).map(([groupName, options]) => (_jsx("optgroup", { label: groupName, children: options.map((option) => {
                    ...
                    if (hasDbType) {
                        ...
                        return (_jsx("option", { value: option.value, children: displayLabel }, option.value));
                    }
