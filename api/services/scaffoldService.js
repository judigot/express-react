...
export const scaffoldService = async (data) => {
    ...
    const { framework, backendDir, frontendDir, dbConnection, backendUrl } = formData;
    ...
    try {
        ...
        if (isBackendDirValid &&
            isFrontendDirValid &&
            backendDirPath === frontendDirPath) {
            const combinedFolderStructure = mergeArrayOfObjects(folderStructures[framework], folderStructures.frontend, 'name');
...
function checkBackendUrlValidity(backendUrl, schemaInfo) {
    return new Promise((resolve) => {
        try {
            ...
            request(`${backendUrl}/${changeCase(schemaInfo[0].tableName).snakeCasePlural}`, (res) => {
