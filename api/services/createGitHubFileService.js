...
export const createGitHubFileService = async (data) => {
    const { publicRepoURL, filePath, content, githubToken, branch = 'main', commitMessage, isBinary = false, } = data;
    if (!publicRepoURL) {
    ...
    try {
        ...
        const defaultCommitMessage = fileSha !== undefined && fileSha !== ''
            ? `Update ${filePath}`
            : `Create ${filePath}`;
        const updateParams = {
            owner: repoInfo.owner,
            repo: repoInfo.repo,
            path: filePath,
            message: commitMessage !== undefined && commitMessage !== ''
                ? commitMessage
