  ...
  async restore({{primaryKey}}: number) {
    return prisma.{{tableName}}.update({
      ...
      data: { deleted_at: undefined },
        ...
        const hasDeletedAt = columnsInfo.filter((columnInfo) => columnInfo.column_name.toLowerCase() === 'deleted_at' ||
            columnInfo.column_name.toLowerCase() === 'deletedat');
        if (hasDeletedAt.length) {
            template = template.replace('{{softDeleteFunctions}}', softDeleteFunctionsTemplate);
        }
        else {
