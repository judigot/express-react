...
const updateOrCreateSection = (content, startMarker, endMarker, newContent) => {
    ...
    if (startId !== -1 && endId !== -1) {
        return (content.slice(0, startId + startMarker.length) +
            '\n' +
            newContent +
            '\n' +
            content.slice(endId));
...
`;
    ...
    if (!template.includes('// Import start')) {
        ...
        template = updateOrCreateSection(template, '// Import start', '// Import end', importStatements);
    }
    if (!template.includes('// Bind start')) {
        ...
        template = updateOrCreateSection(template, '// Bind start', '// Bind end', bindStatements);
    }
    const replacements = {};
